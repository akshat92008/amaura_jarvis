...
class CompanyStore:
    ...
    def _initialize_or_validate_audit_hash_chain(self) -> None:
        ...
        if hash_chain_only or fully_signed:
            check = self._audit_chain_check_rows(rows, require_checkpoint=False)
            if not check["ok"]:
                self._audit_integrity_fault = (
    ...
    def _audit_chain_check_rows(self, rows: list[sqlite3.Row], *, require_checkpoint: bool) -> dict[str, Any]:
        previous = ""
        key = self._audit_key()
        signed_entries = 0
        for row in rows:
        ...
        elif os.environ.get("AMAURA_REQUIRE_EXTERNAL_AUDIT_CHECKPOINT", "0") == "1":
            checkpoint_ok = False
