...
class IntegrationActionController:
    ...
    def stage(self, *, provider: str, operation: str, payload: dict[str, Any], requested_by: str = "jarvis",
              risk: str | None = None, idempotency_key: str = "") -> dict[str, Any]:
        ...
        if _RISK_RANK[resolved_risk] < _RISK_RANK[minimum_risk]:
            raise GovernanceError(
                f"Integration action risk cannot be lower than {minimum_risk} for {key[0]}/{key[1]}"
            )
        ...
        action, inserted = self.store.insert_integration_action({
            "id": action_id, "provider": key[0], "operation": key[1], "payload": payload,
            "payload_hash": self.store.canonical_hash(payload), "idempotency_key": idem,
            "risk": resolved_risk, "status": "awaiting_approval", "requested_by": requested_by,
        })
        if inserted:
            self.store.publish_event("integration.action.awaiting_approval", action["id"],
                                     {"provider": key[0], "operation": key[1], "risk": resolved_risk})
            self.store.audit(requested_by, "stage_integration_action", "integration_action", action["id"],
                             "awaiting_approval", {"provider": key[0], "operation": key[1], "risk": resolved_risk})
        return action
    ...
    def decide(self, action_id: str, *, approve: bool, actor: str, reason: str = "") -> dict[str, Any]:
        ...
        if not approve:
            with self.store.atomic_block():
                action = self.store.approve_integration_action(
                    action_id, actor=actor, approved=False, reason=reason
                )
                self.store.audit(
                    actor,
            ...
            return action
        with self.store.atomic_block():
            action, event = self.store.approve_and_enqueue_integration_action(
                action_id, actor=actor, reason=reason
            )
            self.store.publish_event(
                "integration.action.enqueued", action_id, {"outbox_event_id": event["id"]}
            )
            self.store.audit(
                actor,
