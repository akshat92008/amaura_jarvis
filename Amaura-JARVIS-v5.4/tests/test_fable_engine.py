...
class _DeterministicRouter:
    def generate(self, _prompt, system_prompt=""):
        return {
            "content": "<<THINKING>>fixture plan<<FILES>>[]<<TEST_COMMAND>>python3 main.py",
...
class TestFableEngineIntegration(unittest.TestCase):
    ...
    def test_fable_planner(self):
        planner = FablePlanner(router=_DeterministicRouter())
    ...
    def test_agent_run_fable_reasoning(self):
        ...
        with patch.object(MultiProviderRouter, "generate", _DeterministicRouter().generate):
