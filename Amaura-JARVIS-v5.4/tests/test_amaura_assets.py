...
def test_campaign_schema_validation(factory, store):
    ...
    campaign = factory.create_campaign(
        campaign_id=campaign_id, title="Test Campaign", audience="Engineers", business_objective="Adopt Amaura"
    )
    assert campaign["id"] == campaign_id
    assert campaign["audience"] == "Engineers"

    with pytest.raises(GovernanceError, match="are required"):
        factory.create_campaign(campaign_id="c2", title="T", audience="", business_objective="Obj")
...
def test_asset_schema_validation(factory, store):
    ...
    with pytest.raises(KeyError):
        factory.register_asset(
            campaign_id="invalid", asset_type="blog_post", uri="file:///tmp/blog.md", content=b"Hello"
        )
