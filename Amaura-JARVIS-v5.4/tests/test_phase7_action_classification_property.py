...
MISLEADING_PATH_COMPONENTS = [
    "Desktop", "screen", "browser", "memory", "repo", "image", "video", "write", "read", "screenshot_data",
]

MISLEADING_QUOTED_TOKENS = [
    "screenshot", "delete", "browser", "memory", "capture", "open", "write", "read", "take screenshot",
]
...
def _random_path(misleading: str | None = None) -> str:
    parts = ["Users", "operator", misleading or random.choice(MISLEADING_PATH_COMPONENTS), f"file_{random.randint(100, 999)}"]
...
def test_action_classification_property_1000_cases():
    ...
    for prompt, expected_action, category in cases:
        ...
        if category == "write":
            ...
            assert primary_action == ActionType.STRUCTURED_WORKFLOW, f"Failed for workflow prompt: {prompt} -> got {primary_action}"
            assert not DirectActionRouter._is_screenshot_request(prompt), f"Workflow prompt routed as screenshot: {prompt}"
            ...
            assert primary_action == ActionType.SCREENSHOT_CAPTURE, f"Failed for screenshot prompt: {prompt} -> got {primary_action}"
            ...
            assert DirectActionRouter._is_screenshot_request(prompt) is False, f"Negated/controlled prompt routed as screenshot: {prompt}"
