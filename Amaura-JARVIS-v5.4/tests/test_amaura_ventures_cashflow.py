...
from jarvis.amaura.ventures_cashflow import LANE_PROFILES, CashflowEngine


def _fake_public_fetch(url: str, *, max_length: int = 200_000):
    payload = (
        b"Creators repeatedly ask for faster revision templates and practical exam trackers. "
        b"Students pay for focused resources when the product saves time and avoids repeated mistakes."
    )
    return payload, {
        ...
        evidence=[
            {
                ...
                "claim": "Creators repeatedly ask for faster revision templates",
                "excerpt": "Creators repeatedly ask for faster revision templates",
            },
            {
                "source": "https://two.example/reviews",
                "claim": "Students pay for focused resources",
                "excerpt": "Students pay for focused resources",
            },
        ],
        score_components={},
...
def _provider_evidence(
    external_id: str, *, amount_cents: int, event_type: str = "revenue", units: int = 0
) -> list[dict]:
    ...
    receipt = ProviderReceipt.issue(
        provider="test-store",
        operation=f"financial_{event_type}",
        external_id=external_id,
        idempotency_key=f"idem-{external_id}-{event_type}",
        payload=payload,
        status="confirmed",
    )
        ...
        try:
            ...
            with pytest.raises(GovernanceError):
                engine.create_stream(
                    ...
                    name="Fast Revision Pack",
                    lane="digital_download",
                    platform="owned storefront",
                    offer="Original mistake-tracker PDF and spreadsheet",
                    target_user=opp["target_user"],
                    distribution_channel="organic student content",
                    price_cents=19900,
                    founder_minutes_per_week=30,
                    actor="jarvis",
                )
    ...
    with TemporaryDirectory() as temp:
        ...
        try:
            ...
            engine.record_financial_event(
                ...
                evidence=_provider_evidence("sale-1", amount_cents=39800, event_type="revenue"),
            )
            engine.record_financial_event(
                stream["id"],
                event_type="fee",
                amount_cents=2000,
                source="processor receipt",
                evidence=_provider_evidence("fee-1", amount_cents=2000, event_type="fee"),
            )
            engine.record_financial_event(
                stream["id"],
                event_type="refund",
                amount_cents=19900,
                source="store receipt",
                evidence=_provider_evidence("refund-1", amount_cents=19900, event_type="refund"),
            )
...
def test_cashflow_action_queue_fails_closed_for_external_actions(monkeypatch):
    with TemporaryDirectory() as temp:
        ...
        try:
            ...
            approved = engine.set_action_status(
                external["id"], status="approved", reason="Founder reviewed exact payload", actor=control.founder_id
            )
        ...
        try:
            ...
            with pytest.raises(GovernanceError):
                engine.create_stream(
                    ...
                    lane="kdp_book",
                    platform="KDP",
                    offer="Plagiarize competitor books and use fake reviews",
                    target_user=opp["target_user"],
                    distribution_channel="spam DM",
                    price_cents=29900,
                )
        finally:
            control.close()
    ...
    with TemporaryDirectory() as temp:
        ...
        try:
            ...
            first = engine.record_financial_event(
                ...
                amount_cents=10000,
                source="store",
                evidence=_provider_evidence("same-sale", amount_cents=10000, event_type="revenue"),
            )
            ...
            with pytest.raises(GovernanceError):
                engine.record_financial_event(
                    ...
                    amount_cents=10000,
                    source="store",
                    evidence=_provider_evidence("usd-sale", amount_cents=10000, event_type="revenue"),
                    currency="USD",
                )
        finally:
            control.close()
    ...
    with TemporaryDirectory() as temp:
        ...
        try:
            ...
            second = studio.create_opportunity(
                title="Second cashflow product",
                problem="Students need a second focused resource",
                target_user="NEET students preparing weekly tests",
                product_type="template_pack",
                source="public student discussions",
                evidence=[
                    {
                        "source": "https://three.example/thread",
                        "claim": "Creators repeatedly ask for faster revision templates",
                        "excerpt": "Creators repeatedly ask for faster revision templates",
                    },
                    {
                        "source": "https://four.example/reviews",
                        "claim": "Students pay for focused resources",
                        "excerpt": "Students pay for focused resources",
                    },
                ],
                ...
                monetization="One-time sale",
                distribution_channel="organic",
                strategic_fit="low capital",
            )
            engine = CashflowEngine(control)
            engine.create_stream(
            ...
            with pytest.raises(GovernanceError):
                engine.create_stream(
                    ...
                    platform="store",
                    offer="Original two",
                    target_user=second["target_user"],
                    distribution_channel="organic",
                    price_cents=10000,
                    founder_minutes_per_week=30,
                )
    ...
    with TemporaryDirectory() as temp:
        ...
        try:
            ...
            engine.record_financial_event(
                ...
                event_type="revenue",
                amount_cents=7000,
                source="Founder bank statement",
                evidence=[
                    {"founder_attestation": True, "manual_event_id": "manual-1", "note": "Manual reconciliation"}
                ],
...
def test_cashflow_action_uses_canonical_company_approval(monkeypatch):
    with TemporaryDirectory() as temp:
        ...
        try:
            ...
            external = next(
                a for a in engine.tick(auto_execute=False)["proposals_created"] if a["requires_founder_approval"]
            )
            ...
            approved = engine.set_action_status(
                action["id"], status="approved", reason="Exact payload approved", actor=control.founder_id
            )
            assert approved["status"] == "running"
            assert approved["mission_id"]
...
def test_cashflow_action_payload_mutation_invalidates_approval(monkeypatch):
    with TemporaryDirectory() as temp:
        ...
        try:
            ...
            action = control.store.get_venture_cashflow_action(external["id"])
            control.store.update_venture_cashflow_action(
                action["id"], payload={**action["payload"], "offer": "mutated after request"}
            )
            with pytest.raises(GovernanceError):
...
def test_lane_ranking_learns_from_real_stream_outcomes(monkeypatch):
    with TemporaryDirectory() as temp:
        ...
        try:
            ...
            second = studio.create_opportunity(
                title="Second learned product",
                problem="Students need a compact weekly error review system",
    ...
    with TemporaryDirectory() as temp:
        ...
        try:
            ...
            two = studio.create_opportunity(
                ...
                problem="Students need a compact revision helper",
                target_user="NEET students",
                product_type="template_pack",
                source="public discussion",
                evidence=[
                    {
                        "source": "https://one.example/c",
                        "claim": "Creators repeatedly ask for faster revision templates",
                        "excerpt": "Creators repeatedly ask for faster revision templates",
                    },
                    {
                        "source": "https://two.example/d",
                        "claim": "Students pay for focused resources",
                        "excerpt": "Students pay for focused resources",
                    },
                ],
            ...
            live_attention = sum(
                int(row["founder_minutes_per_week"])
                for row in first_control.store.list_venture_cashflow_streams(limit=10)
    ...
    with TemporaryDirectory() as temp:
        ...
        try:
            ...
            o2 = studio.create_opportunity(
                title="Live cap second",
                problem="Students need another focused helper",
                target_user="NEET students",
                product_type="template_pack",
                source="public discussion",
                evidence=[
                    {
                        "source": "https://one.example/e",
                        "claim": "Creators repeatedly ask for faster revision templates",
                        "excerpt": "Creators repeatedly ask for faster revision templates",
                    },
                    {
                        "source": "https://two.example/f",
                        "claim": "Students pay for focused resources",
                        "excerpt": "Students pay for focused resources",
                    },
                ],
            ...
            s1 = e1.create_stream(
                ...
                platform="store",
                offer="Original",
                target_user=o1["target_user"],
                distribution_channel="organic",
                price_cents=10000,
                founder_minutes_per_week=20,
            )
            s2 = e2.create_stream(
    ...
    with TemporaryDirectory() as temp:
        ...
        try:
            ...
            control.decide_approval(
                action["approval_id"], control.founder_id, "approved", "Approved in central approvals UI"
            )
...
def test_cancelling_cashflow_action_cancels_linked_jarvis_mission(monkeypatch):
    with TemporaryDirectory() as temp:
        ...
        try:
            ...
            engine.tick(auto_execute=True)
            action = next(
                a for a in engine.next_actions(limit=20) if not a["requires_founder_approval"] and a["mission_id"]
            )
            cancelled = engine.set_action_status(
                action["id"], status="cancelled", reason="Founder stopped this experiment", actor=control.founder_id
            )
    ...
    with TemporaryDirectory() as temp:
        ...
        try:
            ...
            fake = control.evidence.put_json(
                ...
                retrieval_metadata={
                    "financial_trust": "provider_verified",
                    "provider": "made-up-store",
                    "external_event_id": "fake-1",
                    "amount_cents": 10000,
                    "event_type": "revenue",
                    "currency": "INR",
                },
            ...
            with pytest.raises(GovernanceError):
                engine.record_financial_event(
                    ...
                    amount_cents=10000,
                    source="vault",
                    evidence=[{"reference": fake.reference}],
                    actor="jarvis",
                )
        finally:
            control.close()
...
def test_failed_cashflow_action_retry_creates_fresh_mission(monkeypatch):
    ...
    with TemporaryDirectory() as temp:
        ...
        try:
            ...
            first = next(
                a
                for a in engine.tick(auto_execute=True)["action_queue"]
                if not a["requires_founder_approval"] and a["mission_id"]
            )
            ...
            reset = engine.set_action_status(
                failed["id"], status="proposed", reason="retry with fresh mission", actor="jarvis"
            )
    ...
    with TemporaryDirectory() as temp:
        ...
        try:
            ...
            second = engine.record_financial_event(
                ...
                event_type="revenue",
                amount_cents=10000,
                source="bank changed note",
                evidence=[
                    {
                        "founder_attestation": True,
                        "manual_event_id": "bank-txn-1",
                        "note": "same statement row, annotated",
                    }
                ],
                actor=control.founder_id,
            )
...
def test_failed_provider_receipt_cannot_count_as_financial_truth(monkeypatch):
    ...
    with TemporaryDirectory() as temp:
        ...
        try:
            ...
            receipt = ProviderReceipt.issue(
                provider="test-store",
                operation="financial_revenue",
                external_id="declined-1",
                idempotency_key="declined-idem",
                payload=payload,
                status="failed",
            )
            with pytest.raises(GovernanceError):
...
def test_v53_database_migrates_financial_and_action_trust_columns(monkeypatch):
    ...
    with TemporaryDirectory() as temp:
        ...
        try:
            finance_cols = {
                row[1] for row in store._connection.execute("PRAGMA table_info(venture_financial_events)").fetchall()
            }
            action_cols = {
                row[1] for row in store._connection.execute("PRAGMA table_info(venture_cashflow_actions)").fetchall()
            }
