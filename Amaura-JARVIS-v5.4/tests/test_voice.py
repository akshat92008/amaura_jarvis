...
class TestStreamingVoice(unittest.TestCase):
    def setUp(self):
        ...
        self._orig_data_dir = os.environ.get("AMAURA_DATA_DIR")
        os.environ["AMAURA_DATA_DIR"] = self._temp_dir

    def tearDown(self):
        if self._orig_data_dir is not None:
            os.environ["AMAURA_DATA_DIR"] = self._orig_data_dir
        else:
            os.environ.pop("AMAURA_DATA_DIR", None)
    ...
    def test_voice_session_state_machine(self):
        # This is a state-machine unit test, not a live cognition probe.  Keep
        # it deterministic and ensure the suite never waits on a configured
        # provider while validating voice lifecycle transitions.
        engine = DuplexVoiceEngine(command_handler=lambda text: f"acknowledged: {text}")
