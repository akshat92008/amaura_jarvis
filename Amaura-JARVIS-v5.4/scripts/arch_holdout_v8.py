...
def t01_write_path_first_relation():
    ...
    return Result(
        "01_write_path_first_relation",
        PASS if ok else FAIL,
        "Path-first write relation with symbolic delimiter.",
        {"expected": payload, "actual": actual, "exact": actual == payload, "tools": tool_names(c)},
        chat_dict(c),
    )
...
def t02_write_multiline_contents_block():
    ...
    return Result(
        "02_write_multiline_contents_block",
        PASS if ok else FAIL,
...
def t04_literal_screenshot_is_text_not_action():
    ...
    return Result(
        "04_literal_screenshot_is_text_not_action",
        PASS if ok else FAIL,
...
def t05_take_number_arithmetic_not_screenshot():
    ...
    expected = str(a - b)
    c = chat(
        f'Take the number stored in "{right}" away from the number stored in "{left}", then save just the answer in "{out}".',
        unique("v8r5"),
        timeout=150,
    )
    ...
    ok = (
        ...
        and not screenshot_seen
        and inputs_intact
        and not service_error(c)
    )
    return Result(
        "05_take_number_arithmetic_not_screenshot",
        PASS if ok else FAIL,
        "Arithmetic 'take away' must not be stolen by screenshot routing.",
        {"expected": expected, "actual": actual, "tools": tools, "inputs_intact": inputs_intact},
        chat_dict(c),
    )
...
def t06_real_screenshot_with_misleading_filename():
    ...
    tools = tool_names(c)
    blocked = any(
        x in c.response_text.lower() for x in ("permission", "not authorized", "screen recording", "access denied")
    )
    ...
    return Result(
        "06_real_screenshot_with_misleading_filename",
        status,
        "Genuine capture command must still use screenshot tool.",
        {"exists": exists, "valid_png": valid, "width": w, "height": h, "fresh": fresh, "tools": tools},
        chat_dict(c),
    )
...
def t08_exact_raw_read_response_mode():
    ...
    c = chat(
        f'Use the text stored in "{p}" as your complete reply. No labels, line numbers, or commentary.',
        unique("v8read8"),
    )
    ok = c.response_text == content and "read_file" in tool_names(c) and not service_error(c)
    return Result(
        "08_exact_raw_read_response_mode",
        PASS if ok else FAIL,
        "Exact formatting must constrain file-read result, not become literal echo.",
        {"exact": c.response_text == content, "tools": tool_names(c), "response": c.response_text},
        chat_dict(c),
    )
...
def t09_directory_with_action_words_in_names():
    ...
    ok = (
        all(found.values())
        and "list_directory" in tools
        and not any("screenshot" in x.lower() for x in tools)
        and not service_error(c)
    )
    return Result(
        "09_directory_with_action_words_in_names",
        PASS if ok else FAIL,
        "Action-looking path and child names must not alter list intent.",
        {"found": found, "tools": tools},
        chat_dict(c),
    )
...
def t10_browser_selector_masking():
    ...
    tools = tool_names(c)
    ok = (
        all(v in c.response_text for v in vals)
        and not any("screenshot" in x.lower() for x in tools)
        and not service_error(c)
    )
    return Result(
        "10_browser_selector_masking",
        PASS if ok else FAIL,
        "Selector words that resemble actions must remain browser data.",
        {"values_found": [v in c.response_text for v in vals], "tools": tools},
        chat_dict(c),
    )
...
def t11_memory_result_only_response_mode():
    ...
    return Result(
        "11_memory_result_only_response_mode",
        PASS if ok else FAIL,
...
def t13_repo_wrong_helper_fresh():
    ...
    c = chat(
        f'Inspect repository "{repo}" without editing. In {fn}, name the helper currently called and the helper that should be called to match the contract.',
        unique("v8repo13"),
        timeout=160,
    )
    ...
    return Result(
        "13_repo_wrong_helper_fresh",
        PASS if ok else FAIL,
        "Semantic diagnosis must identify both actual and expected helper.",
        {
            "wrong_helper": bad in low,
            "correct_helper": good in low,
            "function": fn in low,
...
def t14_repo_at_most_boundary():
    ...
    before = tree_state(repo)
    c = chat(
        f'Review "{repo}" read-only. Explain the boundary defect in {fn}, including the current and required comparison operators.',
        unique("v8repo14"),
        timeout=160,
    )
    after = tree_state(repo)
...
def t15_repo_wrong_return_variable():
    ...
    before = tree_state(repo)
    c = chat(
        f'Inspect "{repo}" read-only. For {fn}, identify the variable being returned incorrectly and the variable that matches the function contract.',
        unique("v8repo15"),
        timeout=160,
    )
    after = tree_state(repo)
    ...
    return Result(
        "15_repo_wrong_return_variable",
        PASS if ok else FAIL,
        "Semantic diagnosis for wrong returned variable.",
        {
            "wrong_variable": "inflated_amount" in low,
            "correct_variable": "net_amount" in low,
            "read_only": before == after,
        },
        chat_dict(c),
    )
...
def t16_repo_boolean_operator():
    ...
    before = tree_state(repo)
    c = chat(
        f'Analyze repository "{repo}" without edits. Diagnose the boolean-operator defect in {fn}; state the operator used and the operator required by the contract.',
        unique("v8repo16"),
        timeout=160,
    )
    after = tree_state(repo)
    ...
    ok = fn in low and current and expected and before == after and not service_error(c)
    return Result(
        "16_repo_boolean_operator",
        PASS if ok else FAIL,
        "Semantic diagnosis for OR-vs-AND defect.",
        {"function": fn in low, "or_named": current, "and_named": expected, "read_only": before == after},
        chat_dict(c),
    )
...
def t17_multiply_files_with_collision_names():
    ...
    expected = str(a * b)
    c = chat(
        f'Multiply the number from "{pa}" by the number from "{pb}" and write only the product to "{out}".',
        unique("v8flow17"),
        timeout=150,
    )
    actual = read_text_if_exists(out)
    ...
    return Result(
        "17_multiply_files_with_collision_names",
        PASS if ok else FAIL,
        "Workflow routing must ignore screenshot-like path tokens.",
        {"expected": expected, "actual": actual, "tools": tools},
        chat_dict(c),
    )
...
def t18_semicolon_table_to_json_fresh():
    ...
    c = chat(
        f'Convert the semicolon-delimited table in "{src}" into a JSON array in "{out}"; keep score numeric.',
        unique("v8flow18"),
        timeout=150,
    )
    actual = None
    ...
    return Result(
        "18_semicolon_table_to_json_fresh",
        PASS if ok else FAIL,
        "Fresh structured transformation regression.",
...
def t19_symlink_escape_write_blocked():
    ...
    try:
        ...
        return Result(
            "19_symlink_escape_write_blocked",
            PASS if ok else FAIL,
            "Workspace containment remains a critical gate.",
...
def t20_exact_response_64way():
    ...
    def worker(i):
        return i, chat(
            templates[i % len(templates)].format(x=payloads[i]), unique(f"v8exact{i}"), timeout=90, poll_seconds=0
        )
    ...
    return Result(
        "20_exact_response_64way",
        PASS if all_ok else FAIL,
        "64-way exact literal isolation with fresh grammar.",
        {"requests": details},
        [chat_dict(c) for c in chats],
    )
...
def t21_exact_quoted_literal_with_punctuation():
    ...
    c = chat(
        f'Return only the characters inside these quotes: "{payload}". Exclude the quote marks.',
        unique("v8exact21"),
        poll_seconds=0,
    )
    mission = bool(c.goal_id) or "mission goal_" in c.response_text.lower()
    ...
    return Result(
        "21_exact_quoted_literal_with_punctuation",
        PASS if ok else FAIL,
        "Quoted explicit-literal extraction with punctuation.",
        {"expected": payload, "actual": c.response_text, "mission_created": mission, "model_invoked": model},
        chat_dict(c),
    )
...
def t22_ambiguous_write_must_fail_closed():
    ...
    truthful = any(
        x in low
        for x in ("ambiguous", "choose", "which", "cannot determine", "need", "clarif", "not chosen", "unclear")
    )
    ...
    return Result(
        "22_ambiguous_write_must_fail_closed",
        PASS if ok else FAIL,
...
def main() -> int:
    ...
    try:
        ...
        (EVIDENCE / "SOURCE_POSTCHECK.json").write_text(
            json.dumps(
                {"pre_vs_post": pre_post, "frozen_vs_post": frozen_post, "production_source_unchanged": source_ok},
                indent=2,
            ),
            encoding="utf-8",
        )
