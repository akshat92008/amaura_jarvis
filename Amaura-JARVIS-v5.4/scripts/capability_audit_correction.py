...
def main():
    ...
    # ----------------------------------------------------
    from jarvis.amaura.registry import V1_AGENTS, AGENTS_BY_ID
    ...
    # ----------------------------------------------------
    from jarvis.tools.registry import ALL_TOOL_DEFINITIONS, ALL_DISPATCH
    ...
    subprocess.run([
        ...
        '-c:v', 'libx264', '-c:a', 'aac', str(ffmpeg_input)
    ...
    probe_res = subprocess.run([
        ...
        '-show_format', '-show_streams', str(ffmpeg_output)
    ...
    slides_spec = [
        {'title': 'Amaura JARVIS v5.4 Capability Audit', 'bullets': ['Executive Summary', 'Qualification Scope', 'Correction Pass 2026-08-13']},
        {'title': 'System Architecture & Cognition', 'bullets': ['OmniRoute Cognition Gateway', 'Multi-Agent OS', 'Trust Foundation & Audit Chain']},
        {'title': 'Workforce & Capabilities', 'bullets': ['57 Autonomous Company OS Employees', '137 Registered & Routable Tools', '22 Governed Workflow Templates']},
        {'title': 'Verification & Compliance', 'bullets': ['Independent Verification Engine', 'Durable Proof Logs & Receipts', 'Fail-Closed Governance Boundary']},
        {'title': 'Final Qualification Verdict', 'bullets': ['Full Capability Qualification Certified', 'Zero Defects In Canonical Test Suite', 'All Systems Qualified & Operational']}
    ]

    ppt_res_str = execute_tool('create_presentation', {'output_path': str(ppt_path), 'title': 'Amaura JARVIS v5.4 Qualification', 'slides': slides_spec})
    ...
    doc_ppt_res_str = execute_tool('create_presentation', {'output_path': str(doc_ppt_path), 'title': 'Factsheet Presentation', 'slides': slides_spec[:3]})
    doc_ppt_res = json.loads(doc_ppt_res_str)
    ...
    (out_dir / "document_generation.json").write_text(json.dumps({
        "PPT": ppt_evidence,
