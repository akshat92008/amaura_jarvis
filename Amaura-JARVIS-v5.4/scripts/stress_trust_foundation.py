...
def audit_writer(db: str, checkpoint: str, key: str, worker: int, count: int) -> None:
    ...
    with CompanyStore(db) as store:
        for index in range(count):
            store.audit(
                f"worker-{worker}", "stress_append", "trust", str(index), "allowed", {"worker": worker, "index": index}
            )
            ...
            store.create_venture_experiment_with_slot(
                {
                    ...
                    "product_name": f"product-{os.getpid()}",
                    "hypothesis": "bounded",
                    "stage": "validating",
                    "timebox_days": 14,
                    "budget_cents": 0,
                    "primary_metric": "validated_users",
                    "target_value": 10.0,
                    "kill_threshold": 1.0,
                },
                max_active=1,
...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="amaura-trust-stress-") as tmp:
        ...
        workers = [
            ctx.Process(target=audit_writer, args=(str(db), str(checkpoint), audit_key, i, 20)) for i in range(32)
        ]
        ...
        with sqlite3.connect(db) as connection:
            ...
            for sequence, actor, action, resource_type, resource_id, outcome, details, created_at in rows:
                ...
                connection.execute(
                    "UPDATE audit_logs SET actor=?,prev_hash=?,entry_hash=? WHERE sequence=?",
                    (actor, previous, digest, sequence),
                )
                previous = digest
            connection.commit()
        with CompanyStore(db) as store:
        ...
        first = vault.put_text(
            "same bytes", source="https://one.example/report", worker_id="worker-a", task_id="task-a"
        )
        second = vault.put_text(
            "same bytes", source="https://two.example/report", worker_id="worker-a", task_id="task-a"
        )
