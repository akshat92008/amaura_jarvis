...
def main():
    ...
    # Phase 19: Document
    doc_cap = next(
        (c for c in capabilities if "doc" in c.get("key", "").lower() or "pdf" in c.get("key", "").lower()), None
    )
    ...
    rag_present = rag_cap is not None or any(
        "vector" in str(c).lower() or "memory" in str(c).lower() for c in capabilities
    )
    ...
    med_cap = next(
        (c for c in capabilities if c.get("key") in {"ffmpeg", "whisper", "kokoro", "voice", "vision"}), None
    )
    med_present = med_cap is not None or any(
        "voice" in str(c).lower() or "vision" in str(c).lower() for c in capabilities
    )
