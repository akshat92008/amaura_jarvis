...
tracked = ["JARVIS","AMAURA","OMNIROUTE","NVIDIA","OPENAI","ANTHROPIC",
           "TELEGRAM","SMTP","COMFY","SEARX","WHATSAPP","GOOGLE_CLIENT","GROQ","OPENROUTER"]
env_audit = {k: ("SET" if v else "EMPTY") for k,v in os.environ.items()
             if any(p in k.upper() for p in tracked)}
...
try:
    ...
    omniroute_status = -1; omniroute_models = 0; print(f"  OmniRoute error: {e}")
...
freeze = {
    ...
    "git": {"head": git_head, "tree": git_tree,
            "status_lines": len(git_status.splitlines()),
            "status": git_status, "recent_log": git_log},
    "python_version": py_version, "platform": uname_out,
    "packages_count": len(packages), "packages": packages,
    "environment_variables": env_audit,
    "omniroute": {"url":"http://127.0.0.1:20128","status":omniroute_status,"model_count":omniroute_models},
    "binaries": binaries, "agy_version": agy_ver,
    "tool_registry": {"counts_by_category": tool_counts,
                      "total": tool_counts.get("total",0), "all_names": tool_names},
}
