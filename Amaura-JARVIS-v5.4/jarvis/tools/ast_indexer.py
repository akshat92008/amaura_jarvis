    ...
    },
    {
        ...
        "function": {
            ...
            "parameters": {
                "type": "object",
                "properties": {
                    "root_dir": {"type": "string", "description": "Directory path to analyze.", "default": "."}
                },
            },
        },
    },
    ...
    def _parse_js_file(self, file_path: str, rel_path: str):
        try:
            ...
            for idx, line in enumerate(lines, 1):
                ...
                func_match = re.search(r"function\s+([A-Za-z0-9_]+)\s*\(([^)]*)\)", line) or re.search(
                    r"(?:const|let|var)\s+([A-Za-z0-9_]+)\s*=\s*(?:\([^)]*\)|[A-Za-z0-9_]+)\s*=>", line
                )
                import_match = re.search(r'import\s+.*from\s+[\'"]([^\'"]+)[\'"]', line) or re.search(
                    r'require\([\'"]([^\'"]+)[\'"]\)', line
                )
...
def index_codebase_ast(root_dir: str = ".") -> str:
    ...
    for s in symbols[:15]:
        if s["type"] == "class":
            preview.append(
                f"• `class {s['name']}` ({s['file']}:{s['line']}) — Methods: {', '.join(s.get('methods', [])[:5])}"
            )
        else:
...
def search_symbol(query: str, root_dir: str = ".") -> str:
    ...
    for s in matches:
        if s["type"] == "class":
            ...
            results.append(
                f"📦 **class `{s['name']}`** in `{s['file']}:{s['line']}`\n  Methods: {methods}\n  Docstring: {s['docstring'][:100]}"
            )
        else:
            args = ", ".join(s.get("args", []))
            results.append(
                f"⚡ **def `{s['name']}({args})`** in `{s['file']}:{s['line']}`\n  Docstring: {s['docstring'][:100]}"
            )
