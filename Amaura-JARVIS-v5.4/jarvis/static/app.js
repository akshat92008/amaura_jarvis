...
document.addEventListener("DOMContentLoaded", () => {
    ...
    // ── OMNIROUTE / COGNITION GATEWAY STATUS HUD ──────────────────────────
    async function fetchCognitionStatus() {
        try {
            const res = await fetch("/api/amaura/cognition/status", {
                headers: amauraOperatorHeaders(),
            });
            if (!res.ok) return;
            const s = await res.json();
            renderCognitionStatus(s);
        } catch (e) {
            console.debug("Cognition status unavailable:", e);
        }
    }

    function renderCognitionStatus(s) {
        // Locate or lazily create the cognition card in the HUD sidebar
        let card = document.getElementById("cognition-status-card");
        if (!card) {
            card = document.createElement("div");
            card.id = "cognition-status-card";
            card.className = "hud-status-card";
            card.style.cssText = [
                "margin:8px 0",
                "padding:10px 12px",
                "border-radius:8px",
            ...
            // Insert just below the HUD sidebar header (first child)
            const sidebar = document.getElementById("hud-sidebar");
            if (sidebar) sidebar.prepend(card);
        }

        const gatewayReady = s.available === true && s.status === "READY";
        const statusColour = gatewayReady
            ? "var(--success-color,#22c55e)"
            : s.status === "BLOCKED"
            ? "var(--error-color,#ef4444)"
            : "var(--warning-color,#f59e0b)";
        const statusIcon = gatewayReady ? "●" : "◉";
        const gatewayLabel = escapeHtml(s.gateway || s.provider || "unknown");
        const modelLabel = escapeHtml(s.model || "unset");
        const fallbackLabel = s.fallback_model ? escapeHtml(s.fallback_model) : "—";
        const statusLabel = escapeHtml(s.status || "UNKNOWN");
        const reasonLabel = s.reason ? ` <span style="opacity:.55">(${escapeHtml(s.reason)})</span>` : "";

        card.innerHTML = `
            <div style="display:flex;align-items:center;gap:6px;margin-bottom:5px">
                <span style="color:${statusColour};font-size:14px">${statusIcon}</span>
                <strong style="letter-spacing:.04em">Cognition Gateway</strong>
                <span style="margin-left:auto;padding:1px 7px;border-radius:20px;
                    background:${statusColour}20;color:${statusColour};
                    font-size:10px;letter-spacing:.05em">${statusLabel}</span>
            </div>
            <div style="display:grid;grid-template-columns:auto 1fr;gap:1px 8px;opacity:.85">
                <span style="opacity:.6">Provider</span><span>${gatewayLabel}${reasonLabel}</span>
                <span style="opacity:.6">Model</span><span>${modelLabel}</span>
                <span style="opacity:.6">Fallback</span><span>${fallbackLabel}</span>
            </div>
        `;
    }
    ...
    async function fetchMemoryList() {
        try {
            const res = await fetch("/api/memory", { headers: { ...jarvisHeaders(), ...amauraOperatorHeaders() } });
            const data = await res.json();
    ...
    function setupEventListeners() {
        ...
        addMemoryBtn.onclick = async () => {
            ...
            try {
                await fetch("/api/memory", {
                    method: "POST",
                    headers: { ...jarvisHeaders({ "Content-Type": "application/json" }), ...amauraOperatorHeaders() },
        ...
        clearMemoryBtn.onclick = async () => {
            if (confirm("Are you sure you want to reset JARVIS memory?")) {
                await fetch("/api/memory", { method: "DELETE", headers: { ...jarvisHeaders(), ...amauraOperatorHeaders() } });
                fetchMemoryList();
