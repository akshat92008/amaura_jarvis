...
class GoogleCalendarAdapter:
    ...
    def __init__(self, *, token_provider: OAuthTokenProvider | None = None, transport=request_json, receipt_key: str | None = None) -> None:
    ...
    def create_event(self, *, summary: str, start: dict[str, str], end: dict[str, str], idempotency_key: str,
                     description: str = "", attendees: list[str] | None = None, calendar_id: str = "primary") -> Any:
        ...
        payload: dict[str, Any] = {
            "summary": summary.strip(), "description": description.strip(), "start": start, "end": end,
        }
        ...
        def attempt(token: str):
            return self.transport(endpoint, method="POST", payload=payload,
                                  headers={"Authorization": f"Bearer {token}", "X-Amaura-Idempotency-Key": idempotency_key}, timeout=30)
        ...
        return _receipt(provider="google-calendar", operation="create_calendar_event", external_id=external_id,
                        thread_id=str(response.get("htmlLink", "")), idempotency_key=idempotency_key,
                        payload={**payload, "calendar_id": calendar_id.strip() or "primary"}, status="created", key=self.receipt_key)


class GoogleDriveAdapter:
    endpoint = "https://www.googleapis.com/upload/drive/v3/files?uploadType=multipart&fields=id,webViewLink,name,mimeType"

    def __init__(self, *, token_provider: OAuthTokenProvider | None = None, transport=request_bytes, receipt_key: str | None = None) -> None:
        self.tokens = token_provider or OAuthTokenProvider("AMAURA_GOOGLE")
        self.transport = transport
        self.receipt_key = receipt_key
    ...
    def upload_file(self, *, path: str, idempotency_key: str, folder_id: str = "", name: str = "", mime_type: str = "application/octet-stream") -> Any:
        if not self.configured:
        ...
        def attempt(token: str):
            return self.transport(self.endpoint, method="POST", body=body,
                                  headers={"Authorization": f"Bearer {token}", "Content-Type": f"multipart/related; boundary={boundary}",
                                           "X-Amaura-Idempotency-Key": idempotency_key}, timeout=60, max_response_bytes=2_000_000)
        ...
        receipt_payload = {"name": metadata["name"], "folder_id": folder_id.strip(), "mime_type": mime_type,
                           "source_sha256": __import__("hashlib").sha256(raw).hexdigest()}
        return _receipt(provider="google-drive", operation="upload_drive_file", external_id=external_id,
                        thread_id=str(response.get("webViewLink", "")), idempotency_key=idempotency_key,
                        payload=receipt_payload, status="uploaded", key=self.receipt_key)
...
class GitHubAdapter:
    ...
    def _headers(self, idempotency_key: str) -> dict[str, str]:
        return {"Authorization": f"Bearer {self.token}", "Accept": "application/vnd.github+json",
                "X-GitHub-Api-Version": "2022-11-28", "X-Amaura-Idempotency-Key": idempotency_key}

    def create_issue(self, *, owner: str, repo: str, title: str, body: str, idempotency_key: str, labels: list[str] | None = None) -> Any:
        if not self.configured:
        ...
        status, response, _ = self.transport(f"https://api.github.com/repos/{owner}/{repo}/issues", method="POST",
                                             payload=payload, headers=self._headers(idempotency_key), timeout=30)
        if status != 201:
        ...
        return _receipt(provider="github", operation="create_github_issue", external_id=external_id,
                        thread_id=str(response.get("html_url", "")), idempotency_key=idempotency_key,
                        payload={"owner": owner, "repo": repo, **payload}, status="created", key=self.receipt_key)

    def dispatch_workflow(self, *, owner: str, repo: str, workflow_id: str, ref: str, idempotency_key: str,
                          inputs: dict[str, str] | None = None) -> Any:
        if not self.configured:
            raise GovernanceError("GitHub token is not configured")
        owner, repo = self._repo(owner, repo)
        ...
        status, response, headers = self.transport(
            ...
            method="POST", payload=payload, headers=self._headers(idempotency_key), timeout=30)
        if status != 204:
        ...
        return _receipt(provider="github", operation="dispatch_github_workflow", external_id=external_id,
                        idempotency_key=idempotency_key,
                        payload={"owner": owner, "repo": repo, "workflow_id": workflow_id.strip(), **payload},
                        status="accepted", key=self.receipt_key)


class PostHogAdapter:
    def __init__(self, *, host: str | None = None, project_key: str | None = None, transport=request_json, receipt_key: str | None = None) -> None:
        self.host = (host if host is not None else os.environ.get("AMAURA_POSTHOG_HOST", "https://us.i.posthog.com")).rstrip("/")
        self.project_key = (project_key if project_key is not None else os.environ.get("AMAURA_POSTHOG_PROJECT_KEY", "")).strip()
        self.transport = transport
        self.receipt_key = receipt_key
    ...
    def capture(self, *, event: str, distinct_id: str, properties: dict[str, Any], idempotency_key: str) -> Any:
        ...
        payload = {"api_key": self.project_key, "event": event.strip(), "properties": {"distinct_id": distinct_id.strip(), **properties}}
        if not payload["event"] or not distinct_id.strip():
            raise GovernanceError("Analytics event and distinct id are required")
        status, response, _ = self.transport(f"{self.host}/capture/", method="POST", payload=payload,
                                             headers={"X-Amaura-Idempotency-Key": idempotency_key}, timeout=20)
        if status not in {200, 201}:
            raise GovernanceError(f"PostHog capture failed with HTTP {status}")
        return _receipt(provider="posthog", operation="capture_analytics_event",
