...
from jarvis.amaura.models import ContentAsset, ContentCampaign, GovernanceError
...
class ContentFactory:
    ...
    def create_campaign(
        self, *, campaign_id: str, title: str, audience: str, business_objective: str, config: dict | None = None
    ) -> dict:
    ...
    def publication_readiness(self, campaign_id: str) -> dict:
        ...
        licence_issues = [
            asset["id"]
            for asset in assets
            if urlsplit(asset["uri"]).scheme in {"http", "https"} and (not asset["licence"] or not asset["source_url"])
        ]
        ...
        return {
            "campaign_id": campaign_id,
            "ready": ready,
            "missing_approved_asset_types": missing,
            "licence_issues": licence_issues,
    ...
    def record_metrics(
        ...
        *,
        ...
        if any(
            isinstance(value, bool) or not isinstance(value, (int, float)) or value < 0 for value in metrics.values()
        ):
