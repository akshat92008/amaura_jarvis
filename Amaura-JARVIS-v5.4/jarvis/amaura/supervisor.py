...
class AmauraSupervisor:
    ...
    def tick(self, *, workflow_id: str | None = None, dispatch_outbox: bool = True) -> dict[str, Any]:
        ...
        with self.control.telemetry.trace(
            ...
            return self._tick(workflow_id=workflow_id, dispatch_outbox=dispatch_outbox)

    def _tick(self, *, workflow_id: str | None = None, dispatch_outbox: bool = True) -> dict[str, Any]:
        ...
        try:
            ...
            self.control.telemetry.alert(
                ...
                details={
                    "run_id": run["id"],
                    "goal_id": task.get("parent_id") or task.get("workflow_id") or "",
                    "task_id": task["id"],
                    "task_title": task.get("title", ""),
                    "task_owner": task.get("owner_id", ""),
                    "task_state": task.get("state", outcome),
                    "failure_stage": "tool_execution",
                    "exception_type": type(exc).__name__,
                    "exception_message": str(exc)[:500],
                    "selected_capability": task.get("action_type", ""),
                    "retry_attempt": run.get("attempt", 1),
                    "replan_attempt": (task.get("metadata") or {}).get("replan_count", 0),
